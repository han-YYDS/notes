...
`std::set` 和 `std::unordered_set` 是 C++ 标准库中的两种不同类型的集合容器，它们有以下几个区别：

1. 存储顺序：
   1. `std::set` 是==有序==集合，它按照元素的值进行排序并保持有序状态。
   2. 而 `std::unordered_set` 是==无序==集合，元素存储的顺序并不依赖于元素的值，而是根据哈希函数进行存储和检索。
2. 检索效率：
   1. `std::set` 使用==红黑树==（一种自平衡二叉搜索树）实现，因此它具有较快的有序检索能力，查找元素的时间复杂度为 O(logN)，其中 N 是集合的大小。
   2. 相比之下，`std::unordered_set` 使用==哈希表==实现，查找元素的平均时间复杂度为 O(1)，最坏情况下为 O(N)。
3. 插入和删除效率：
   1. 由于 `std::set` 维护有序状态需要进行平衡操作，因此在插入和删除元素时，它的性能较差，平均时间复杂度为 O(logN)。
   2. 而 `std::unordered_set` 的插入和删除操作具有较高的效率，平均时间复杂度为 O(1)。
4. 内存占用：
   1. `std::set` 使用红黑树来存储元素，因此在存储上会占用==较多的额外内存空间==。
   2. 而 `std::unordered_set` 使用哈希表，可能会占用较少的内存空间，但在哈希冲突较多时，可能需要更多的空间。
5. 迭代器稳定性：
   1. `std::set` 的迭代器在集合修改操作（插入、删除等）后仍然有效，不会失效。
   2. 而 `std::unordered_set` 的迭代器在集合修改操作后可能会==失效==，因为哈希表的重新哈希可能导致元素重新分布。

根据具体的需求，选择使用 `std::set` 还是 `std::unordered_set` 取决于对集合操作的性能要求和对元素排序的需求。如果需要有序集合，对插入和删除操作的性能要求不是很高，或者对元素迭代器的稳定性有要求，可以选择 `std::set`。如果不需要有序性，对插入和删除操作有较高的性能要求，并且不关心元素的顺序，可以选择 `std::unordered_set`。



unordered和order的区别:

一个反常,即有序的容器查找起来比无序的慢,因为unordered用的是hash,相当于用空间换时间

有序访问set: 即当将元素insert入set之后,如果采用迭代器遍历,得到的顺序是有序的(字典序)
...
当调用增强for循环时,如果修改了容器中的元素,插入或者删除,则有可能产生迭代器失效
