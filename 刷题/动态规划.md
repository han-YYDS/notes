...
for(int i = 1; i < weight.size(); i++) { // 遍历物品
    for(int j = 0; j <= bagweight; j++) { // 遍历背包容量
        if (j < weight[i]) dp[i][j] = dp[i - 1][j];  // 不能拿起物品i的情况
        else dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - weight[i]] + value[i]); // 1.能拿起但不如不拿的情况 2.能拿起而且拿起更好的情况
    }
}
```
...
之前所考虑的: 对于之前的排列组合,如果遇到了一个性价比更高的物品,获取该物品需要从之前的状态中放弃一些物品,这种淘汰的机制如何实现?

dp[i - 1] [j - weight[i]] + value[i]) 这里的 之前状态和组合无关,只和 weight有关,在dp中是没有记录组合的,只有对于在当前容量为j下的最优价值结果
...
> sum和target都是给定的
>
> 同时 left = (target+sum)/2, 所以问题转化为,在数组中挑选出子集,其和为(target+sum)/2

```cpp
// 那么 横向为背包容量的遍历,纵向为当前可供选择的货物
class Solution {
public:
    int findTargetSumWays(vector<int>& nums, int S) {
        ...
        if ((S + sum) % 2 == 1) return 0; // 此时没有方案,没有除尽
        int bagSize = (S + sum) / 2;
        vector<int> dp(bagSize + 1, 0);
        dp[0] = 1; // 当容量为0时,如果此时满足要求,那么仍能生效
        for (int i = 0; i < nums.size(); i++) {
